...
func _set_encoding_type(value: String) -> void:
	...
	for character in value:
		is_alphanumeric = is_alphanumeric && DeckBuildQrCodeUtils.ALPHANUMERIC_CHARACTERS.has(character)
...
func _encode_numeric(value: String) -> void:
	...
	for index in data.size():
		...
		qr_data_list.append_array(DeckBuildQrCodeUtils.convert_to_binary(data[index], size))
...
func _encode_alphanumeric(value: String) -> void:
	...
	for index in range(0, ceil(value.length() / 2.0)):
		var first_value: int = DeckBuildQrCodeUtils.ALPHANUMERIC_CHARACTERS[value[index * 2]]
		var second_value: int = -1

		if index * 2 + 1 < value.length():
			second_value = DeckBuildQrCodeUtils.ALPHANUMERIC_CHARACTERS[value[index * 2 + 1]]

		var result: Array
		if second_value != -1:
			result = DeckBuildQrCodeUtils.convert_to_binary(first_value * 45 + second_value, 11)
		else:
			result = DeckBuildQrCodeUtils.convert_to_binary(first_value, 6)
...
func _encode_bytes(value: String) -> void:
	...
	var byte_array = value.to_utf8_buffer()
	for byte in byte_array:
		qr_data_list.append_array(DeckBuildQrCodeUtils.convert_to_binary(byte))
...
func _set_info_segments() -> void:
	...
	var length_bits = DeckBuildQrCodeUtils.convert_to_binary(qr_data_length, length_bits_size)
...
func _set_error_correction() -> void:
	...
	var rs = DeckBuildQrCodeReedSolomonGenerator.new(block_ecc_len)
	...
	for block_index in range(qr_data_list.size()):
		...
		for index in block_bytes.size():
			block_bytes[index] = DeckBuildQrCodeUtils.convert_to_decimal(block_bytes[index])

		block_bytes = rs.get_remainder(block_bytes)

		for byte in block_bytes:
			ecc_data_list[block_index].append_array(DeckBuildQrCodeUtils.convert_to_binary(byte, 8))
